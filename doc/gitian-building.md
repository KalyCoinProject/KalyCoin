Gitian building
================
*Setup instructions for a Gitian build of Kalycoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Kalycoin
...
- [Building Kalycoin Core](#building-kalycoin-core)
...
Clone the git repositories for kalycoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/kalycoinproject/kalycoin --recursive
git clone https://github.com/kalycoinproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/kalycoin/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the kalycoin repository under 'Fetch and create inputs' to install sources which require
...
To build Kalycoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the kalycoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kalycoin/.git/
...
For example:
```bash
URL=https://github.com/laanwj/kalycoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit kalycoin=${COMMIT} --url kalycoin=${URL} ../kalycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kalycoin=${COMMIT} --url kalycoin=${URL} ../kalycoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kalycoin=${COMMIT} --url kalycoin=${URL} ../kalycoin/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-kalycoin:
./bin/gbuild --commit kalycoin=${COMMIT},cpp-eth-kalycoin=${ETHCOMMIT} --url kalycoin=${URL},cpp-eth-kalycoin=${ETHURL} ../kalycoin/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the kalycoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../kalycoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/kalycoinproject/kalycoin-detached-sigs.git

BTCPATH=/some/root/path/kalycoin
SIGPATH=/some/root/path/kalycoin-detached-sigs

./bin/gbuild --url kalycoin=${BTCPATH},signature=${SIGPATH} ../kalycoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kalycoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kalycoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/kalycoin-osx-build.assert
```
...
[kalycoinproject/gitian.sigs](https://github.com/kalycoinproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@kalycoin.io) and he will commit them.
