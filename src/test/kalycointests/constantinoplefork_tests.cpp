...
//    initState();
    ...
    txs.push_back(createKalycoinTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createKalycoinTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createKalycoinAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<KalycoinTransaction> txsCall;
    txsCall.push_back(createKalycoinTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<KalycoinTransaction> txsMint;
    txsMint.push_back(createKalycoinTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<KalycoinTransaction> txsbalance;
    txsbalance.push_back(createKalycoinTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
//    initState();
    ...
    // Create contract
    std::vector<KalycoinTransaction> txs;
    txs.push_back(createKalycoinTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createKalycoinAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<KalycoinTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createKalycoinTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
