...
extern std::unique_ptr<KalycoinState> globalState;
...
inline void initState(){
    ...
    const std::string dirKalycoin = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<KalycoinState>(new KalycoinState(dev::u256(0), KalycoinState::openDB(dirKalycoin, hashDB, dev::WithExisting::Trust), dirKalycoin + "/kalycoinDB", dev::eth::BaseState::Empty));
...
inline dev::Address createKalycoinAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline KalycoinTransaction createKalycoinTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    KalycoinTransaction txEth;
    if(recipient == dev::Address()){
        txEth = KalycoinTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = KalycoinTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<KalycoinTransaction> txs){
