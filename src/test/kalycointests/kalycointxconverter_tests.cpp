...
void checkResult(bool isCreation, std::vector<KalycoinTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    KalycoinTxConverter converter(transaction, NULL);
    ExtractKalycoinTX kalycoinTx;
    BOOST_CHECK(converter.extractionKalycoinTransactions(kalycoinTx));
    std::vector<KalycoinTransaction> result = kalycoinTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(kalycointxconverter_tests, TestingSetup)
