...
Token::Token()
{
    ...
    lstMandatory.append(KalycoinToken::paramAddress());
    lstMandatory.append(KalycoinToken::paramDatahex());
    QStringList lstOptional;
    lstOptional.append(KalycoinToken::paramSender());
    ...
    lstMandatory.append(KalycoinToken::paramDatahex());
    lstOptional.clear();
    lstOptional.append(KalycoinToken::paramAmount());
    lstOptional.append(KalycoinToken::paramGasLimit());
    lstOptional.append(KalycoinToken::paramGasPrice());
    lstOptional.append(KalycoinToken::paramSender());
    lstOptional.append(KalycoinToken::paramBroadcast());
    lstOptional.append(KalycoinToken::paramChangeToSender());
    lstOptional.append(KalycoinToken::paramPsbt());
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                Token::ToKalycoinAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
                Token::ToKalycoinAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
