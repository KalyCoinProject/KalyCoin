...
#define paternVersion "kalycoin-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

KalycoinVersionChecker::KalycoinVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

KalycoinVersionChecker::~KalycoinVersionChecker()
{

}

bool KalycoinVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> KalycoinVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(KLC_RELEASES)));
    QEventLoop event;
...
Version KalycoinVersionChecker::getMaxReleaseVersion()
{
