...
#include <qt/kalycoinversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    kalycoinVersionChecker = new KalycoinVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Kalycoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kalycoin: URIs)"));
    ...
    KRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/krctoken"), tr("&KRC Tokens"), this);
    KRCTokenAction->setStatusTip(tr("KRC Tokens (send, receive or add Tokens in list)"));
    KRCTokenAction->setToolTip(KRCTokenAction->statusTip());
    KRCTokenAction->setCheckable(true);
    KRCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
    tabGroup->addAction(KRCTokenAction);
    ...
    connect(KRCTokenAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(KRCTokenAction, SIGNAL(triggered()), this, SLOT(gotoTokenPage()));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Kalycoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Kalycoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a kalycoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Kalycoin command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->addAction(KRCTokenAction);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && kalycoinVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(KLC_RELEASES, KLC_RELEASES);
            QString message(tr("New version of Kalycoin wallet is available on the Kalycoin source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    KRCTokenAction->setEnabled(enabled);
...
{
    KRCTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Kalycoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
