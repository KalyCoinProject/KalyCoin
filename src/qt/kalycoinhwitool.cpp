...
class KalycoinHwiToolPriv
{
public:
    KalycoinHwiToolPriv(QObject *parent)
    {
...
KalycoinHwiTool::KalycoinHwiTool(QObject *parent) : QObject(parent)
{
    d = new KalycoinHwiToolPriv(this);
}

KalycoinHwiTool::~KalycoinHwiTool()
{
    delete d;
}

bool KalycoinHwiTool::enumerate(QList<HWDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    devices.clear();
    std::vector<LedgerDevice> vecDevices;
    if(KalycoinLedger::instance().enumerate(vecDevices, stake))
    {
    ...
    {
        d->strError = QString::fromStdString(KalycoinLedger::instance().errorMessage());
    }

    return devices.size() > 0;
}

bool KalycoinHwiTool::isConnected(const QString &fingerprint, bool stake)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    bool ret = KalycoinLedger::instance().isConnected(strFingerprint, stake);
    if(!ret) d->strError = QString::fromStdString(KalycoinLedger::instance().errorMessage());
    return ret;
}

bool KalycoinHwiTool::getKeyPool(const QString &fingerprint, int type, const QString& path, bool internal, QString &desc)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = KalycoinLedger::instance().getKeyPool(strFingerprint, type, strPath, internal, d->from, d->to, strDesc);
...
bool KalycoinHwiTool::getKeyPool(const QString &fingerprint, int type, const QString &path, QStringList &descs)
{
    LOCK(cs_ledger);
    bool ret = true;
    QString desc;
...
bool KalycoinHwiTool::getKeyPoolPKH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::LEGACY, path, descs);
}

bool KalycoinHwiTool::getKeyPoolP2SH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::P2SH_SEGWIT, path, descs);
}

bool KalycoinHwiTool::getKeyPoolBech32(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::BECH32, path, descs);
}

bool KalycoinHwiTool::signTx(const QString &fingerprint, QString &psbt)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strPsbt = psbt.toStdString();
    bool ret = KalycoinLedger::instance().signTx(strFingerprint, strPsbt);
    psbt = QString::fromStdString(strPsbt);
    if(!ret) d->strError = QString::fromStdString(KalycoinLedger::instance().errorMessage());
    return ret;
}

bool KalycoinHwiTool::signMessage(const QString &fingerprint, const QString &message, const QString &path, QString &signature)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = KalycoinLedger::instance().signMessage(strFingerprint, strMessage, strPath, strSignature);
    signature = QString::fromStdString(strSignature);
    if(!ret) d->strError = QString::fromStdString(KalycoinLedger::instance().errorMessage());
    return ret;
}

bool KalycoinHwiTool::signDelegate(const QString &fingerprint, QString &psbt)
{
...
QString KalycoinHwiTool::errorMessage()
{
...
bool KalycoinHwiTool::isStarted()
{
    return d->fStarted;
}

void KalycoinHwiTool::wait()
{
    if(d->fStarted)
    {
...
bool KalycoinHwiTool::rescanBlockchain(int startHeight, int stopHeight)
{
    if(!d->model) return false;
...
bool KalycoinHwiTool::importAddresses(const QString &desc)
{
    if(!d->model) return false;
...
bool KalycoinHwiTool::importMulti(const QStringList &descs)
{
    bool ret = true;
...
bool KalycoinHwiTool::finalizePsbt(const QString &psbt, QString &hexTx, bool &complete)
{
    if(!d->model) return false;
...
bool KalycoinHwiTool::sendRawTransaction(const QString &hexTx, QVariantMap& variantMap)
{
    if(!d->model) return false;
...
bool KalycoinHwiTool::decodePsbt(const QString &psbt, QString &decoded)
{
    if(!d->model) return false;
...
void KalycoinHwiTool::setModel(WalletModel *model)
{
    d->model = model;
}

bool KalycoinHwiTool::execRPC(ExecRPCCommand *cmd, const QMap<QString, QString> &lstParams, QVariant &result, QString &resultJson)
{
    d->strError.clear();
...
void KalycoinHwiTool::addError(const QString &error)
{
    if(d->strError != "")
        d->strError += "\n";
    d->strError += error;
...
QString KalycoinHwiTool::derivationPathPKH()
{
    std::string path = KalycoinLedger::instance().derivationPath((int)OutputType::LEGACY);
    return QString::fromStdString(path);
}

QString KalycoinHwiTool::derivationPathP2SH()
{
    std::string path = KalycoinLedger::instance().derivationPath((int)OutputType::P2SH_SEGWIT);
    return QString::fromStdString(path);
}

QString KalycoinHwiTool::derivationPathBech32()
{
    std::string path = KalycoinLedger::instance().derivationPath((int)OutputType::BECH32);
    return QString::fromStdString(path);
}
