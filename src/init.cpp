 ...
 */
static const char* BITCOIN_PID_FILENAME = "kalycoind.pid";
...
void Shutdown(NodeContext& node)
{
    ...
    util::ThreadRename("kalycoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/kalycoinproject/kalycoin>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateKalycoin");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path kalycoinStateDir = GetDataDir() / "stateKalycoin";
                bool fStatus = fs::exists(kalycoinStateDir);
                const std::string dirKalycoin(kalycoinStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsKalycoinstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<KalycoinState>(new KalycoinState(dev::u256(0), KalycoinState::openDB(dirKalycoin, hashDB, dev::WithExisting::Trust), dirKalycoin, existsKalycoinstate));
                dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(kalycoinStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setKalycoinSchedule(kalycoinDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
