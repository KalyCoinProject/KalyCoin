...
KalycoinDelegation& GetKalycoinDelegation()
{
    static KalycoinDelegation kalycoinDelegation;
    return kalycoinDelegation;
}
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, BlockValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view)
{
    ...
    if (nHeight >= nOfflineStakeHeight && !Params().GetConsensus().delegationsAddress.IsNull())
    {
        ...
        KalycoinDelegation& kalycoinDelegation = GetKalycoinDelegation();
        if(!kalycoinDelegation.ExistDelegationContract())
        ...
        bool verifiedDelegation = kalycoinDelegation.VerifyDelegation(address, delegation);
...
bool GetDelegationFeeFromContract(const uint160& address, uint8_t& fee)
{
    Delegation delegation;
    KalycoinDelegation& kalycoinDelegation = GetKalycoinDelegation();
    bool ret = kalycoinDelegation.GetDelegation(address, delegation);
    if(ret) ret &= kalycoinDelegation.VerifyDelegation(address, delegation);
    if(ret)
    {
        fee = delegation.fee;
    }
