...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setKalycoinSchedule(kalycoinDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = kalycoinDGP.getBlockSize(nHeight);
    minGasPrice = kalycoinDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = kalycoinDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    KalycoinTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractKalycoinTX resultConverter;
    if(!convert.extractionKalycoinTransactions(resultConverter)){
    ...
    for(KalycoinTransaction kalycoinTransaction : kalycoinTransactions){
        ...
        if(bceResult.usedGas + kalycoinTransaction.gas() > softBlockGasLimit){
        ...
        if(kalycoinTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, kalycoinTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            kalycoinDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = kalycoinDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                kalycoinDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                kalycoinDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            kalycoinDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            kalycoinDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    KalycoinDelegation kalycoinDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                kalycoinDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = kalycoinDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    kalycoinDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    kalycoinDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                kalycoinDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(kalycoinDelegations.GetDelegation(address, delegation) && KalycoinDelegation::VerifyDelegation(address, delegation))
                    {
...
public:
...

    {
        ...
        std::string threadName = "kalycoinstake";
...
void StakeKalycoins(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
