...
const char * const BITCOIN_CONF_FILENAME = "kalycoin.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Kalycoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Kalycoin
    // Mac: ~/Library/Application Support/Kalycoin
    // Unix: ~/.kalycoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kalycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Kalycoin";
#else
    // Unix
    return pathRet / ".kalycoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Kalycoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Kalycoin Core Developers";
    }
