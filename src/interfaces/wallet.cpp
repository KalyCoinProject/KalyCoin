...
public:
    ...
    bool isUnspentAddress(const std::string &kalycoinAddress) override
    {
        auto locked_chain = m_wallet->chain().lock();
        LOCK(m_wallet->cs_wallet);
        ...
        for (const COutput& out : vecOutputs)
        {
            ...
            if(fValidAddress && EncodeDestination(address) == kalycoinAddress && out.tx->tx->vout[out.i].nValue)
            {
                return true;
            }
        }
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(keyID)
            {
                ...
                contractRet = m_kalycoinDelegation.ExistDelegationContract() ? m_kalycoinDelegation.GetDelegation(address, delegation) : false;
                if(contractRet)
                {
                    validated = m_kalycoinDelegation.VerifyDelegation(address, delegation);
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        {
            uint160 address(*keyID);
            details.c_contract_return = m_kalycoinDelegation.ExistDelegationContract() ? m_kalycoinDelegation.GetDelegation(address, delegation) : false;
            if(details.c_contract_return)
            {
                details.c_entry_exist = m_kalycoinDelegation.VerifyDelegation(address, delegation);
            ...
            if(decoded_psbt.tx->HasOpCall())
            {
                ...
                for(size_t i = 0; i < decoded_psbt.tx->vout.size(); i++){
                    ...
                    if(v.scriptPubKey.HasOpCall()){
                        ...
                        if(KalycoinDelegation::IsAddBytecode(data))
                        {
                            std::string hexStaker;
                            if(!KalycoinDelegation::GetUnsignedStaker(data, hexStaker))
                            {
    ...
    bool setAddDelegationData(std::string& psbt, const std::map<int, SignDelegation>& signData, std::string& error) override
    {
        ...
        for (auto it = signData.begin(); it != signData.end(); it++)
        {
            ...
            if(v.scriptPubKey.HasOpCall()){
                ...
                v.scriptPubKey.GetData(data);
                CScript scriptRet;
                if(KalycoinDelegation::SetSignedStaker(data, PoD) && v.scriptPubKey.SetData(data, scriptRet))
                {
    ...
    KalycoinDelegation m_kalycoinDelegation;
};
