...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // kalycoin
    uint256 hashUTXORoot; // kalycoin
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // kalycoin
        READWRITE(hashUTXORoot); // kalycoin
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot.SetNull(); // kalycoin
        hashUTXORoot.SetNull(); // kalycoin
    ...
    virtual bool IsProofOfStake() const //kalycoin
    {
    ...
    CBlockHeader& operator=(const CBlockHeader& other) //kalycoin
    {
        if (this != &other)
        {
...
public:
    ...
    std::pair<COutPoint, unsigned int> GetProofOfStake() const //kalycoin
    {
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // kalycoin
        block.hashUTXORoot   = hashUTXORoot; // kalycoin
