...
void CWallet::StakeKalycoins(bool fStake, CConnman* connman)
{
    ::StakeKalycoins(fStake, this, connman, stakeThread);
}
...
void CWallet::StartStake(CConnman *connman)
{
    if(IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS))
    {
        m_enabled_staking = !m_ledger_id.empty() && KalycoinLedger::instance().toolExists();
    }
    else
    {
