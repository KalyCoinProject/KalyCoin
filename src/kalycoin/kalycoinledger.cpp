...
using namespace KalycoinLedger_NS;

class KalycoinLedgerPriv
{
public:
    KalycoinLedgerPriv()
    {
        ...
        if(!toolExists)
        {
            LogPrintf("KalycoinLedger(): HWI tool not found %s\n", toolPath);
        }
    }
...
KalycoinLedger::KalycoinLedger():
    d(0)
{
    d = new KalycoinLedgerPriv();
}
...
KalycoinLedger &KalycoinLedger::instance()
{
    static KalycoinLedger device;
    return device;
}

bool KalycoinLedger::signCoinStake(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
...
bool KalycoinLedger::signBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &vchSig)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool KalycoinLedger::isConnected(const std::string &fingerprint, bool stake)
{
...
bool KalycoinLedger::enumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool KalycoinLedger::signTx(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool KalycoinLedger::signMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &signature)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool KalycoinLedger::getKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &desc)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
std::string KalycoinLedger::errorMessage()
{
    LOCK(cs_ledger);
...
bool KalycoinLedger::toolExists()
{
    return d->toolExists;
}

bool KalycoinLedger::isStarted()
{
    return d->fStarted;
}

void KalycoinLedger::wait()
{
    if(d->fStarted)
    {
...
bool KalycoinLedger::beginSignTx(const std::string &fingerprint, std::string &psbt)
{
...
bool KalycoinLedger::endSignTx(const std::string &, std::string &psbt)
{
...
bool KalycoinLedger::beginSignBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool KalycoinLedger::endSignBlockHeader(const std::string &, const std::string &, const std::string &, std::vector<unsigned char> &vchSig)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool KalycoinLedger::beginEnumerate(std::vector<LedgerDevice> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
    arguments << "enumerate";
...
bool KalycoinLedger::endEnumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    ...
    for(size_t i = 0; i < jsonDevices.size(); i++)
    {
        ...
        bool isStakeApp = device.app_name == "Kalycoin Stake" || device.app_name == "Kalycoin Stake Test";
        if(isStakeApp == stake)
        {
...
bool KalycoinLedger::beginSignMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool KalycoinLedger::endSignMessage(const std::string &, const std::string &, const std::string &, std::string &signature)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool KalycoinLedger::beginGetKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &)
{
    // Get the output type
    std::string descType;
    switch (type) {
...
bool KalycoinLedger::endGetKeyPool(const std::string &, int, const std::string& , bool, int, int, std::string &desc)
{
    // Decode command line results
...
std::string KalycoinLedger::derivationPath(int type)
{
