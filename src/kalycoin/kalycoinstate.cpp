...
#include <kalycoin/kalycoinstate.h>
...
KalycoinState::KalycoinState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = KalycoinState::openDB(_path + "/kalycoinDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

KalycoinState::KalycoinState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute KalycoinState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, KalycoinTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createKalycoinAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            kalycoin::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, KalycoinTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, KalycoinTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> KalycoinState::vins() const // temp
{
...
void KalycoinState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* KalycoinState::vin(dev::Address const& _a) const
{
    return const_cast<KalycoinState*>(this)->vin(_a);
}

Vin* KalycoinState::vin(dev::Address const& _addr)
{
...
// void KalycoinState::commit(CommitBehaviour _commitBehaviour)
// {
...
void KalycoinState::kill(dev::Address _addr)
{
...
void KalycoinState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void KalycoinState::deleteAccounts(std::set<dev::Address>& addrs){
...
void KalycoinState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void KalycoinState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void KalycoinState::validateTransfersWithChangeLog(){
...
void KalycoinState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!KalycoinState::addressInUse(delegationsAddress)){
        KalycoinState::createContract(delegationsAddress);
        KalycoinState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)}, KalycoinState::version(delegationsAddress));
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
