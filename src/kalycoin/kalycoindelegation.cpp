...
class KalycoinDelegationPriv
{
public:
    KalycoinDelegationPriv():
        m_pfDelegations(0),
    ...
    virtual ~KalycoinDelegationPriv()
    {
        if(m_pfDelegations)
...
KalycoinDelegation::KalycoinDelegation():
    priv(0)
{
    priv = new KalycoinDelegationPriv();
}
...
bool KalycoinDelegation::GetDelegation(const uint160 &address, Delegation &delegation) const
{
...
bool KalycoinDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool KalycoinDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> KalycoinDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void KalycoinDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool KalycoinDelegation::ExistDelegationContract() const
{
    // Delegation contract exist check
    return globalState && globalState->addressInUse(priv->delegationsAddress);
}

std::string KalycoinDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool KalycoinDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{
...
bool KalycoinDelegation::IsAddBytecode(const std::vector<unsigned char> &data)
{
...
bool KalycoinDelegation::GetUnsignedStaker(const std::vector<unsigned char> &data, std::string &hexStaker)
{
...
bool KalycoinDelegation::SetSignedStaker(std::vector<unsigned char> &data, const std::string &base64PoD)
{
    if(!IsAddBytecode(data))
        return false;
