#include <kalycoin/kalycoinDGP.h>
...
void KalycoinDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool KalycoinDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule KalycoinDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t KalycoinDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t KalycoinDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t KalycoinDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t KalycoinDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool KalycoinDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void KalycoinDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void KalycoinDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void KalycoinDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void KalycoinDGP::createParamsInstance(){
...
dev::Address KalycoinDGP::getAddressForBlock(unsigned int blockHeight){
...
void KalycoinDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void KalycoinDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void KalycoinDGP::parseStorageOneUint64(uint64_t& value){
...
void KalycoinDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule KalycoinDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
