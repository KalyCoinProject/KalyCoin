...
UniValue transactionReceiptToJSON(const KalycoinTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue CallToContract(const UniValue& params)
{
    ...
    if(params.size() >= 3){
        CTxDestination kalycoinSenderAddress = DecodeDestination(params[2].get_str());
        if (IsValidDestination(kalycoinSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&kalycoinSenderAddress);
    ...
    for(size_t i = 0; i < list.size(); i++)
    {
        ...
        for(size_t i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                ToKalycoinAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].get_str().substr(24);
                ToKalycoinAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
