...
#include <util/signstr.h>
#include <kalycoin/kalycoinledger.h>
...
#if defined(NDEBUG)
# error "Kalycoin cannot be compiled without assertions."
#endif
...
std::unique_ptr<KalycoinState> globalState;
...
bool fAddressIndex = false; // kalycoin
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = kalycoinDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = kalycoinDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        KalycoinTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractKalycoinTX resultConverter;
        if(!converter.extractionKalycoinTransactions(resultConverter)){
        ...
        std::vector<KalycoinTransaction> kalycoinTransactions = resultConverter.first;
        std::vector<EthTransactionParams> kalycoinETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(KalycoinTransaction kalycoinTransaction : kalycoinTransactions){
            sumGas += kalycoinTransaction.gas() * kalycoinTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, kalycoinTransaction.gasPrice());
            } else {
                txMinGasPrice = kalycoinTransaction.gasPrice();
            }
            VersionVM v = kalycoinTransaction.getVersion();
            if(v.format!=0)
            ...
            if(kalycoinTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(kalycoinTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(kalycoinTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += kalycoinTransaction.gas();
            ...
            if(v.rootVM!=0 && (uint64_t)kalycoinTransaction.gasPrice() < minGasPrice)
        ...
        if(!CheckMinGasPrice(kalycoinETP, minGasPrice))
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-txns-small-gasprice");

        if(count > kalycoinTransactions.size())
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s KLC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = 6.39189 * COIN / blocktimeDownscaleFactor;
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // kalycoin
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // kalycoin
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    KalycoinTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = KalycoinTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = KalycoinTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<KalycoinTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path kalycoinDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(kalycoinDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(kalycoinDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(KalycoinTransaction& tx : txs){
        //validate VM version
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, KalycoinTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool KalycoinTxConverter::extractionKalycoinTransactions(ExtractKalycoinTX& kalycointx){
    ...
    std::vector<KalycoinTransaction> resultTX;
    ...
    kalycointx = std::make_pair(resultTX, resultETP);
    return true;
}

bool KalycoinTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool KalycoinTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
KalycoinTransaction KalycoinTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    KalycoinTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = KalycoinTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = KalycoinTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t KalycoinTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // kalycoin
    KalycoinDGP kalycoinDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setKalycoinSchedule(kalycoinDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = kalycoinDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = kalycoinDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = kalycoinDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // kalycoin
...
///////////////////////////////////////////////////////////////////////////////////////// kalycoin
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            KalycoinTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractKalycoinTX resultConvertKalycoinTX;
            if(!convert.extractionKalycoinTransactions(resultConvertKalycoinTX)){
            ...
            if(!CheckMinGasPrice(resultConvertKalycoinTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertKalycoinTX.first, blockGasLimit, pindex->pprev);
            ...
            for(KalycoinTransaction& qtx : resultConvertKalycoinTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertKalycoinTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertKalycoinTX.first[k].from(),
                        resultConvertKalycoinTX.first[k].to(),
                        ...
                        resultConvertKalycoinTX.first[k].getNVout(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().MineBlocksOnDemand()) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(75000000 + ((pindex->nHeight - 5000) * 25)) * COIN){
...
bool CChainState::ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // kalycoin
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // kalycoin
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // kalycoin
            globalState->setRootUTXO(oldHashUTXORoot); // kalycoin
...
bool SignBlockHWI(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    KalycoinLedger &device = KalycoinLedger::instance();
...
bool SignBlockLedger(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    {
        std::string errorMessage = KalycoinLedger::instance().errorMessage();
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // kalycoin
        uint32_t sizeBlockDGP = kalycoinDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Kalycoin PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        KalycoinTxConverter convert(tx, NULL, NULL, contractflags);
